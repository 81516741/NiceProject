...
-(void)readyForData
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        selfWeak.datas = [WBTimelineItem localData];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            selfWeak.progressView.hidden = true;
        });
    });
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.pictureClick = ^(NSArray<NSURL *> * picURLs,NSInteger selectedIndex){
        ...
        for (NSURL * url in picURLs) {
            [selfWeak.photos addObject:[MWPhoto photoWithURL:url]];
        }
        MWPhotoBrowser *browser = [[MWPhotoBrowser alloc] initWithDelegate:selfWeak];
        //不显示分享复制按钮
        browser.displayActionButton = NO;
        [browser setCurrentPhotoIndex:selectedIndex];
        // Present
        [selfWeak.navigationController pushViewController:browser animated:YES];
    };
